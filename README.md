...
- `stream` - Stream real-time events. Options: `--types <list>`, `--json`, `--format agent`, `--webhook-endpoint <url>`, `--webhook-body <template>`.
...
Use `--format agent` for a single-line, agent-friendly output like:
